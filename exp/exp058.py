...
class Config:
    ...
    linear_dim: int = 128
    linear_vocab_dim: int = 32  # if == 0: disable
    linear_perplexity_dim: int = 64
    dropout: float = 0.2
    dropout_stack: float = 0.1
    dropout_vocab: float = 0.2
    dropout_self_attn: float = 0.2
    batch_size: int = 16
    ...
    tcn_module_num: int = 2
    tcn_module: nn.Module = TemporalConvNet
    tcn_module_kernel_size: int = 3
    ...
    num_heads: int = 8
    self_attention_enable: bool = True
    mask_p: float = 0
    max_length: int = 256
...
class CommonLitModule(LightningModule):
    def __init__(self,
        ...
        self.self_attn = nn.MultiheadAttention(self.bert.config.hidden_size,
                                               num_heads=self.cfg.num_heads,
                                               dropout=self.cfg.dropout_self_attn)
        ...
        if "efficientnet" in self.cfg.cnn_model_name:
            self.convnet.conv_stem = nn.Conv2d(self.bert.config.num_hidden_layers+1, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
        if "resnet" in self.cfg.cnn_model_name:
            self.convnet.conv1 = nn.Conv2d(self.bert.config.num_hidden_layers+1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)

        # network cfg
        if self.cfg.linear_vocab_enable:
            hidden_size = self.cfg.linear_vocab_dim * self.cfg.max_length
        elif self.cfg.self_attention_enable:
        ...
        self.linear_vocab = nn.Sequential(
            nn.Linear(self.bert.config.hidden_size, self.cfg.linear_vocab_dim),
            nn.Dropout(self.cfg.dropout),
            self.cfg.activation()
        )
        ...
        self.dropout = nn.Dropout(self.cfg.dropout_vocab)
    ...
    def get_attention_matrix(self, x, attention_mask):
        x_src = x.permute(1, 0, 2)  # (N, Vocab, Emb) -> (Vocab, N, Emb)
        x = self.self_attn(x_src, x_src, x_src, key_padding_mask=attention_mask == 0)[1]  # get attention matrix
        return x
    ...
    def forward(self, input_ids_masked, attention_mask, token_type_ids, input_ids):
        ...
        # base feature
        if self.cfg.linear_vocab_enable:
            x_bert = self.dropout(self.linear_vocab(x[0]).view(len(input_ids), -1))
        elif self.cfg.self_attention_enable:
            x = torch.stack([self.get_attention_matrix(xx, attention_mask) for xx in x[1]])
            x = x.permute(1, 0, 2, 3) # (C, N, X, Y) -> (N, C, X, Y)
            x_bert = self.convnet(x)
        else:
            x_bert = torch.stack([self.dropout_bert_stack(xx) for xx in x[1][-4:]]).mean(dim=0)
            x_bert = torch.sum(
...
if __name__ == "__main__":
    ...
    for model_name in ["resnet18",
                       ...
                       "efficientnet_b1",
                       "efficientnet_b2"]:
        for cnn_pretrained in [True, False]:
            cfg = Config(experiment_name=experiment_name)
            cfg.cnn_pretrained = cnn_pretrained
            cfg.cnn_model_name = model_name
    ...
    for tcn_module_enable in [True]:
        cfg = Config(experiment_name=experiment_name)
        cfg.tcn_module_enable = tcn_module_enable
        main(cfg, folds=folds)
