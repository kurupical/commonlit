...
if __name__ == "__main__":
    ...
    for nlp_model_name in ["funnel-transformer/xlarge-base"]:
        """
        for gradient_clipping in [0.1, 0.2, 0.5]:
        ...
        for rnn_module_indice in [(-1, -2, -3, -4)]:
            cfg = Config(experiment_name=experiment_name)
            cfg.nlp_model_name = nlp_model_name
            cfg = common_config(cfg)
            cfg.word_axis = True
            cfg.rnn_hidden_indice = rnn_module_indice
            main(cfg, folds=folds)
