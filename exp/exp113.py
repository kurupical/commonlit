...
if __name__ == "__main__":
    experiment_name = "selfattn_pooling"
    folds = [0]

    for linear_dim in [128, 256, 512, 1024, 2048]:
        cfg = Config(experiment_name=experiment_name)
        config_large(cfg, nlp_model_name="roberta-large")
        cfg.attention_pool_enable = True
        cfg.linear_final_dim = linear_dim
        cfg.linear_dim = linear_dim // 4
        cfg.kl_div_enable = False
        main(cfg, folds=folds)

    for linear_dim in [128, 256, 512, 1024, 2048]:
        for conv2d_hidden_channel in [8, 16, 32, 64]:
            cfg = Config(experiment_name=experiment_name)
            config_large(cfg, nlp_model_name="roberta-large")
            cfg.cnn_model_name = "SimpleConv2D"
            cfg.linear_final_dim = linear_dim
            cfg.linear_dim = linear_dim // 4
            cfg.conv2d_hidden_channel = conv2d_hidden_channel
            cfg.kl_div_enable = False
            main(cfg, folds=folds)
