...
class CommonLitModule(LightningModule):
    ...
    def forward(self, input_ids, attention_mask):
        if "xlnet" in config.nlp_model_name:
            x = self.bert(input_ids=input_ids, attention_mask=attention_mask, output_hidden_states=True)[0].mean(dim=1)
        else:
...
if __name__ == "__main__":
    ...
    for lr_fc in [5e-5, 1e-4, 5e-4, 1e-3]:
        config = Config(experiment_name=experiment_name)
        config.lr_fc = lr_fc
        main(config, folds=folds)

    for lr_bert in [1e-5, 3e-5, 5e-5]:
        config = Config(experiment_name=experiment_name)
        config.lr_bert = lr_bert
        main(config, folds=folds)

    for dropout_stack in [0, 0.1, 0.2]:
        config = Config(experiment_name=experiment_name)
    ...
    for weight_decay in [0, 0.1]:
        config = Config(experiment_name=experiment_name)
        config.weight_decay = weight_decay
        main(config, folds=folds)

    for linear_dim in [64, 256, 512]:
        config = Config(experiment_name=experiment_name)
        config.linear_dim = linear_dim
        main(config, folds=folds)

    for num_warmup_steps in [16*50, 16*300]:
        config = Config(experiment_name=experiment_name)
        config.num_warmup_steps = num_warmup_steps
        main(config, folds=folds)

    for num_training_steps in [16*1000, 16*1500, 16*2000, 16*3000]:
        config = Config(experiment_name=experiment_name)
        config.num_training_steps = num_training_steps
        main(config, folds=folds)

    for activation in [nn.ReLU, nn.GELU, nn.LeakyReLU]:
        config = Config(experiment_name=experiment_name)
