        ...
        if self.cfg.prep_enable:
            ...
            if self.cfg.fine_tuned_path is not None:
                ...
                if "t5" in self.cfg.nlp_model_name:
                    ...
                    """
                    # T5LayerNormがバグってる
                    for i in range(len(self.bert.encoder.block)):
                        hidden_size = len(self.bert.encoder.block[i].layer[0].layer_norm.weight)
                        eps = self.bert.encoder.block[i].layer[0].layer_norm.variance_epsilon
                        self.bert.encoder.block[i].layer[0].layer_norm = nn.LayerNorm(hidden_size, eps=eps)
                        hidden_size = len(self.bert.encoder.block[i].layer[1].layer_norm.weight)
                        eps = self.bert.encoder.block[i].layer[1].layer_norm.variance_epsilon
                        self.bert.encoder.block[i].layer[1].layer_norm = nn.LayerNorm(hidden_size, eps=eps)

                    hidden_size = len(self.bert.encoder.final_layer_norm.weight)
                    eps = self.bert.encoder.final_layer_norm.variance_epsilon
                    self.bert.encoder.final_layer_norm = nn.LayerNorm(hidden_size, eps=eps)
                    """
                else:
            ...
            try:
                ...
                if cfg.nlp_model_name in ["microsoft/deberta-large"]:
                    ...
                    if fold == 2 and rmse / 3 > 0.484:
                        break
                if cfg.nlp_model_name in ["xlnet-large-cased"]:
                    if fold == 0 and rmse > 0.464:
                        break
                    if fold == 1 and rmse / 2 > 0.461:
                        break
...
if __name__ == "__main__":
    ...
    def common_config(cfg) -> Config:
        ...
        if cfg.nlp_model_name == "studio-ousia/luke-large":
            cfg.reinit_layers = 5
        if cfg.nlp_model_name == "xlnet-large-based":
            cfg.reinit_layers = 5
            cfg.epochs = 3
            cfg.epochs_max = 3
        return cfg
