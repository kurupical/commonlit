        ...
        for fold in folds:
            try:
                ...
                if fold == 0 and model.best_rmse > 0.485:
                    break
                if fold == 0 and model.best_rmse > 0.475 and "large" in cfg.nlp_model_name:
                    break
                ...
                if cfg.nlp_model_name in ["roberta-large", "luke-large"]:
                    ...
                    if fold == 3 and rmse / 4 > 0.474:
                        break
                if "large" in cfg.nlp_model_name and "xlnet" not in cfg.nlp_model_name:
                    if fold == 0 and rmse > 0.475:
                        break
                    if fold == 1 and rmse / 2 > 0.47:
                        break
                    if fold == 2 and rmse / 3 > 0.485:
                        break
                    if fold == 3 and rmse / 4 > 0.479:
                        break
...
if __name__ == "__main__":
    experiment_name = "funnel_medium tune"
    folds = [0, 1, 2, 3, 4]
    ...
    for nlp_model_name in ["funnel-transformer/medium-base"]:
        for weight_decay in [0, 0.01, 0.1]:
            for reinit_layers in [0, 1, 2, 3]:
