...
if __name__ == "__main__":
    ...
    for lr_rnn in [1e-5, 1e-4, 5e-4]:
        ...
        cfg.hidden_stack_enable = True
        main(cfg, folds=folds)
    # epoch(scheduler) tune
    for epochs_max in [10, 12, 15]:
        cfg = Config(experiment_name=experiment_name)
        cfg.rnn_module_num = 1
        cfg.epochs_max = epochs_max
        cfg.prep_enable = True
        cfg.hidden_stack_enable = True
        main(cfg, folds=folds)
    # other models
    for nlp_model in ["bert-base-cased",
                      "bert-base-uncased"]:
        cfg = Config(experiment_name=experiment_name)
        cfg.rnn_module_num = 1
    ...
    for linear_perplexity_dim in [32, 128]:
        cfg = Config(experiment_name=experiment_name)
        cfg.rnn_module_num = 1
    ...
    for multi_dropout_ratio in [0.1, 0.2, 0.5]:
        cfg = Config(experiment_name=experiment_name)
        cfg.rnn_module_num = 1
    ...
    for augmantation_range in [(-2, -0.5), (-1.5, -1)]:
        cfg = Config(experiment_name=experiment_name)
        cfg.rnn_module_num = 1
