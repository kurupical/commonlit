...
class CommonLitModule(LightningModule):
    ...
    def forward(self, input_ids_masked, attention_mask, token_type_ids, input_ids):
        ...
        if not self.cfg.prep_enable:
            ...
            # x[0]: final layer output
            # x[1]: hidden layers
            # x[2]: attention map
            # x[3]: pooler
            if "xlnet" in self.cfg.nlp_model_name:
                if len(x) == 3:
                    x = [x[0], x[1], x[2]]
                if len(x) == 4:  # sanity checkのときだけなぜかlength=4になる...
                    x = [x[0], x[2], x[3]] # xlnet don't have pooler
            else:
                x = [x[0], x[2], x[3], x[1]]
    ...
    with mlflow.start_run() as run:
        ...
        for fold in folds:
            try:
                ...
                if fold == 0 and model.best_rmse > 0.51:
                    break
...
if __name__ == "__main__":
    experiment_name = "bert-base-cased randomsearch tune"
    folds = [0, 1, 2, 3, 4]
    ...
    for i in range(100):
        ...
        cfg.reinit_layers = np.random.choice([2, 3])
        cfg.reinit_pooler = True
        cfg.gradient_clipping = np.random.choice([0.1, 0.2, 0.5, 1])
        cfg.kl_div_enable = False
        cfg.simple_structure = True
        cfg.rnn_module_num = np.random.choice([0, 1])
        cfg.tcn_module_enable = np.random.choice([False, True])
        cfg.hidden_stack_enable = np.random.choice([False, True])
        cfg.attention_pool_enable = np.random.choice([False, True])
        cfg.linear_vocab_enable = np.random.choice([False, True])
        cfg.self_attention_enable = np.random.choice([False, True])
        cfg.crossentropy = np.random.choice([False, True])
        cfg.lr_bert = np.random.choice([1e-5, 2e-5, 5e-5])
        cfg.linear_final_dim = np.random.choice([64, 128, 256, 512, 1024, 2048])
