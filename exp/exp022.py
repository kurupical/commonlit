...
class Config:
    ...
    multi_dropout_ratio: float = 0
    multi_dropout_num: int = 1
    fine_tuned_path: str = "finetuned_model/roberta_base_warmup_5"
...
if __name__ == "__main__":
    ...
    for warmup_ratio in [0, 0.05, 0.1]:
    ...
    for lr_bert_decay in [0.995]:
        cfg = Config(experiment_name=experiment_name)
        cfg.lr_bert_decay = lr_bert_decay
        main(cfg, folds=folds)

    for weight_decay in [1e-3, 1e-1]:
        cfg = Config(experiment_name=experiment_name)
        cfg.weight_decay = weight_decay
        main(cfg, folds=folds)

    for multi_dropout_ratio in [0.1, 0.3, 0.5]:
        cfg = Config(experiment_name=experiment_name)
        cfg.multi_dropout_ratio = multi_dropout_ratio
        cfg.multi_dropout_num = 5
        main(cfg, folds=folds)
    ...
    for fine_tuned_path in ["finetuned_model/roberta_base_warmup_5",
                            "finetuned_model/roberta_base_warmup_10",
                            "finetuned_model/roberta_base_warmup_15"]:
        cfg = Config(experiment_name=experiment_name)
