    ...
    with mlflow.start_run() as run:
        ...
        for fold in folds:
            try:
                ...
                # 足切り1
                if fold == 0 and model.best_rmse > 0.5:
                    break
...
if __name__ == "__main__":
    ...
    for nlp_model_name in ["roberta-large"]:
        ...
        for gradient_clipping in [0.1, 0.5]:
            cfg = Config(experiment_name=experiment_name)
            cfg = common_config(cfg)
        ...
        cfg = Config(experiment_name=experiment_name)
        cfg = common_config(cfg)
        cfg.fine_tuned_path = "finetuned_model/roberta_large_5"
        cfg.nlp_model_name = nlp_model_name
        main(cfg, folds=folds)
        # kldiv
        cfg = Config(experiment_name=experiment_name)
        ...
        # lstm
        for rnn_hidden_indice in [(-1, -2), (-1, -2, -3, -4), (-1, -2, 0, 1)]:
            cfg = Config(experiment_name=experiment_name)
            cfg = common_config(cfg)
            cfg.nlp_model_name = nlp_model_name
        ...
        for rnn_module_shrink_ratio in [0.125, 0.5]:
            cfg = Config(experiment_name=experiment_name)
            cfg = common_config(cfg)
            cfg.nlp_model_name = nlp_model_name
        ...
        # attention
        for cnn_model_name in ["resnet34", "resnet50"]:
            cfg = Config(experiment_name=experiment_name)
            cfg = common_config(cfg)
            cfg.nlp_model_name = nlp_model_name
        ...
        cfg = Config(experiment_name=experiment_name)
        cfg = common_config(cfg)
        cfg.linear_vocab_dim_1 = 1024
        cfg.linear_vocab_dim = 512
        cfg.nlp_model_name = nlp_model_name
        main(cfg, folds=folds)

        # tcn
        for tcn_kernel_size in [4, 8]:
            cfg = Config(experiment_name=experiment_name)
            cfg = common_config(cfg)
            cfg.tcn_module_kernel_size = tcn_kernel_size
            cfg.nlp_model_name = nlp_model_name
            main(cfg, folds=folds)