...
class CommonLitModule(LightningModule):
    ...
    def setup(self, stage=None):
        ...
        if self.cfg.debug:
            df = df.iloc[::30]
        if self.cfg.random_fold:
            df["kfold"] = (df["standard_error"] * 12345).astype(int) % 5
...
if __name__ == "__main__":
    ...
    for nlp_model_name in ["roberta-base"]:
        for fine_tuned_path in ["finetuned_model/roberta_base_warmup_10",
                                None]:
            for lr_fc in [5e-5, 1e-4, 2.5e-4, 5e-4]:
