...
class CommonLitDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        target_mu = torch.tensor(row["target"], dtype=torch.float)
        target_std = torch.tensor(row["standard_error"], dtype=torch.float)
        return input_ids, attention_mask, token_type_ids, target_mu, target_std
...
class Config:
    ...
    fine_tuned_path = "finetuned_model/roberta_base_warmup_5"


def klloss(mu1, std1, mu2, std2):
    return torch.log(std2 / std1) + (std1**2 + (mu1 - mu2)**2) / (2 * std2**2) - 1/2
...
class CommonLitModule(LightningModule):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        std = torch.exp(0.5 * self.linear_std(x))
    ...
    def training_step(self, batch, batch_idx):
        ...
        input_ids, attention_mask, token_type_ids, target_mu, target_std = batch
        mu, std = self.forward(input_ids, attention_mask, token_type_ids)

        loss = klloss(mu1=mu, std1=std/10, mu2=target_mu, std2=target_std/10).mean()
    ...
    def validation_step(self, batch, batch_idx):
        input_ids, attention_mask, token_type_ids, target_mu, target_std = batch
        mu, std = self.forward(input_ids, attention_mask, token_type_ids)
        loss = klloss(mu1=mu, std1=std, mu2=target_mu, std2=target_std).mean()
        self.log('val_loss', loss, prog_bar=True)
        return mu.cpu().detach().numpy().flatten(), target_mu.cpu().detach().numpy().flatten()
    ...
    def configure_optimizers(self):
        ...
        params.append(extract_params(self.linear_mean.named_parameters(), lr=self.cfg.lr_fc, weight_decay=self.cfg.weight_decay, no_decay=False))
        params.append(extract_params(self.linear_mean.named_parameters(), lr=self.cfg.lr_fc, weight_decay=0, no_decay=True))
        params.append(extract_params(self.linear_std.named_parameters(), lr=self.cfg.lr_fc, weight_decay=self.cfg.weight_decay, no_decay=False))
        params.append(extract_params(self.linear_std.named_parameters(), lr=self.cfg.lr_fc, weight_decay=0, no_decay=True))
...
if __name__ == "__main__":
    ...
    experiment_name = "predict_distributions"
    folds = [0, 1, 2, 3, 4]
