...
class Config:
    ...
    final_finetune: bool = True
    final_finetune_decay_ratio: bool = 0.001
    ...
    with mlflow.start_run() as run:
        ...
        for fold in folds:
            try:
                ...
                if cfg.final_finetune:
                    # もう1回
                    cfg = copy.copy(cfg_original)
                    cfg.fold = fold
                    model = CommonLitModule(cfg=cfg,
                                            output_dir=output_dir)
                    # model = model.load_from_checkpoint(f"output/submit_models/exp052_roberta_base_tcn_residual/best_fold{fold}.ckpt")
                    model.load_state_dict(torch.load(f"output/submit_models/exp052_roberta_base_tcn_residual/best_fold{fold}.ckpt")["state_dict"])

                    cfg.lr_tcn *= cfg.final_finetune_decay_ratio
                    cfg.lr_bert *= cfg.final_finetune_decay_ratio
                    cfg.lr_tcn *= cfg.final_finetune_decay_ratio
                    cfg.lr_fc *= cfg.final_finetune_decay_ratio
...
if __name__ == "__main__":
    ...
    folds = [0, 1, 2, 3, 4]
    """
    for final_finetune_decay_ratio in [0.0005, 0.0001]:
        for weight_decay in [0, 0.0001, 0.001, 0.01]:
            cfg = Config(experiment_name=experiment_name)
            cfg.weight_decay = weight_decay
            cfg.final_finetune_decay_ratio = final_finetune_decay_ratio
            main(cfg, folds=folds)
    """
    for final_finetune_decay_ratio in [0.00001, 0.00005]:
        for weight_decay in [0, 0.001, 0.1, 1]:
            cfg = Config(experiment_name=experiment_name)
            cfg.weight_decay = weight_decay
            cfg.final_finetune_decay_ratio = final_finetune_decay_ratio
            main(cfg, folds=folds)
