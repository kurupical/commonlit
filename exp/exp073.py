...
if __name__ == "__main__":
    experiment_name = "roberta tcn tuning"
    folds = [0, 1, 2, 3, 4]

    """
    for tcn_module_num in [1]:
    ...
    for linear_vocab_dim in [1, 2, 4]:
        ...
        main(cfg, folds=folds)
    for training_steps_ratio in [1.2, 1.5, 2]:
        cfg = Config(experiment_name=experiment_name)
        cfg.prep_enable = True
        cfg.hidden_stack_enable = True
        cfg.tcn_module_enable = True
        cfg.training_steps_ratio = training_steps_ratio
        main(cfg, folds=folds)
    """
