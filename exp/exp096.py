...
class CommonLitModule(LightningModule):
    ...
    def resize(self, x, attention_mask):
        resize_func = Resize(256)
        ret = []
        for i in range(len(x)):
            xx = x[i][:, :attention_mask[i].sum(), :attention_mask[i].sum()]
            xx = resize_func.forward(xx)
            ret.append(xx)
        ret = torch.stack(ret)
        return ret
    ...
    def forward(self, input_ids_masked, attention_mask, token_type_ids, input_ids):
        ...
        if self.cfg.self_attention_enable:
            xx = torch.cat([self.dropout_attn(self.resize(xx, attention_mask)) for xx in x[2]], dim=1)
            xx = self.convnet(xx)
...
if __name__ == "__main__":
    experiment_name = "linear_dim=2048"
    folds = [0, 1, 2, 3, 4]
