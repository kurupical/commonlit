...
if __name__ == "__main__":
    experiment_name = "よわモデル tuning(fold 0 only)"
    folds = [0, 1, 2, 3, 4]

    for nlp_model_name in ["distilbert-base-cased",
                           "albert-base-v2"]:
        """
        for reinit_layers in [1, 0, 2, 3, 4]:
        ...
        """
        if "albert" in nlp_model_name:
            reinit_layers = 0
        else:
            reinit_layers = 1

        for lr_bert in [1e-5, 2e-5, 4e-5, 5e-5]:
            cfg = Config(experiment_name=experiment_name)
            cfg.simple_structure = True
            cfg.pooler_enable = False
            cfg.reinit_pooler = False
        ...
        for gradient_clip in [0.1, 0.2, 0.5]:
            cfg = Config(experiment_name=experiment_name)
            cfg.simple_structure = True
            cfg.pooler_enable = False
            cfg.reinit_pooler = False
        ...
        for dropout_bert in [0.05, 0.1, 0.2]:
            cfg = Config(experiment_name=experiment_name)
            cfg.simple_structure = True
            cfg.pooler_enable = False
            cfg.reinit_pooler = False
