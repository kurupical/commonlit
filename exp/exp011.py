...
if __name__ == "__main__":
    ...
    for model in ["roberta-base",
        ...
        for rnn_module_drop_ratio in [1, 0.5, 2]:
        ...
        for rnn_module_num in [1, 2]:
            config = Config(experiment_name=experiment_name)
            config.rnn_module_num = rnn_module_num
            config.nlp_model_name = model
            main(config, folds=folds)

        for rnn_module_dropout in [0.2, 0.5]:
            config = Config(experiment_name=experiment_name)
            config.rnn_module_dropout = rnn_module_dropout
            config.nlp_model_name = model
            main(config, folds=folds)

        for lr_bert in [1e-5, 5e-5]:
            config = Config(experiment_name=experiment_name)
            config.lr_rnn = lr_bert
            config.nlp_model_name = model
            main(config, folds=folds)

        for lr_rnn in [1e-5, 5e-5, 1e-4]:
            config = Config(experiment_name=experiment_name)
            config.lr_rnn = lr_rnn
            config.nlp_model_name = model
            main(config, folds=folds)

        for rnn_module_activation in [nn.ReLU, nn.GELU, nn.ELU]:
            config = Config(experiment_name=experiment_name)
            config.rnn_module_activation = rnn_module_activation
            main(config, folds=folds)
