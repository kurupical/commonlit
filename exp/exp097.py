...
class CommonLitModule(LightningModule):
    def __init__(self,
        ...
        self.conv1d = nn.Sequential(
            nn.Conv1d(self.cfg.linear_final_dim, self.cfg.linear_final_dim, kernel_size=self.cfg.conv1d_kernel_size)
        )


        if self.cfg.prep_enable:
            self.linear1 = nn.Sequential(
                nn.Linear(self.cfg.linear_final_dim + self.cfg.linear_perplexity_dim, self.cfg.linear_dim),
                nn.Dropout(self.cfg.dropout),
                self.cfg.activation()
            )
            ...
            self.linear1 = nn.Sequential(
                nn.Linear(self.cfg.linear_final_dim, self.cfg.linear_dim),
                nn.Dropout(self.cfg.dropout),
                self.cfg.activation()
            )
            self.linear2 = nn.Sequential(
    ...
    def forward(self, input_ids_masked, attention_mask, token_type_ids, input_ids):
        ...
        x_bert = torch.stack(x_bert, dim=2)
        x_bert = self.conv1d(x_bert).mean(dim=2)
        if self.cfg.prep_enable:
    ...
    def configure_optimizers(self):
        ...
        params.append(extract_params(self.conv1d.named_parameters(), lr=self.cfg.lr_fc, weight_decay=self.cfg.weight_decay, no_decay=False))
        params.append(extract_params(self.conv1d.named_parameters(), lr=self.cfg.lr_fc, weight_decay=0, no_decay=True))
...
if __name__ == "__main__":
    ...
    for kernel_size in [4, 5, 6]:
        ...
        cfg.conv1d_kernel_size = kernel_size
        main(cfg, folds=folds)

    for final_dim in [256, 512]:
        cfg = Config(experiment_name=experiment_name)
        cfg = config_large(cfg, nlp_model_name="roberta-large")
        cfg.rnn_module_num = 1
        cfg.self_attention_enable = True
        ...
        cfg.linear_vocab_enable = True
        cfg.hidden_stack_enable = True
        cfg.linear_final_dim = final_dim
        cfg.linear_dim = final_dim//2
        main(cfg, folds=folds)
