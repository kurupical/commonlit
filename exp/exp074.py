...
class CommonLitModule(LightningModule):
    ...
    def forward(self, input_ids_masked, attention_mask, token_type_ids, input_ids):
        ...
        # CNN(無理やり)
        xx = torch.cat([xx for xx in x[1][-self.cfg.conv1d_last_dim:]], dim=2)
...
if __name__ == "__main__":
    experiment_name = "simple 1D-CNN"
    folds = [0, 1, 2, 3, 4]

    for conv1d_last_dim in [4, 3, 2, 1]:
        cfg = Config(experiment_name=experiment_name)
        cfg.conv1d_last_dim = conv1d_last_dim
